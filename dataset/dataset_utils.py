...
def calc_class_weights(gts):
    gt = torch.tensor(gts, dtype=torch.int32)
...
class LPSD_Dataset(Dataset):
    def __init__(self, sldir, partition, imgsz=32, device='cpu', transform=None,task=None):
        if device == -1:
            device = "cpu"
        ...
        print(sldir, partition, len(self.fs))

    def __len__(self):
        return self.nfs

    def __getitem__(self, idx):
        #im = self.transform(self.ims[idx]).to(self.device)#.to(self.device)
        im = self.ims[idx]
        lb = self.lbt[idx]#.to(self.device)
        return im, lb
