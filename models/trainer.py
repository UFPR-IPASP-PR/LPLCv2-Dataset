...
from models.utils import start_log, end_log, log_metrics_json, dict_to_table, log_config
...
def train_torch_model(model, cfg, dataset, save_path, log_cfg=None):
    ...
    dts = LPSD_Dataset(dataset['path'], "train", imgsz=dataset['imgsz'],
                       transform=dataset['transform_config'], device=cfg['use_gpu'])
        ...
        while epoch < max_epochs:
            ...
            with tqdm(train_data, unit="batch") as tepoch:
                for sample in tepoch:
                    ...
                    pds = torch.cat([pds, pd])
                    gts = torch.cat([gts, lb])
    ...
    for epoch_loss, tm in train_epoch(cfg['epochs']+1, log_cfg['method'], log_cfg['steps'], device=cfg['use_gpu']):
        ...
        tem = calc_metrics(model, test_data, "test", class_names=dataset['class_names'],
                           loss=loss, device=cfg['use_gpu'])
        log_metrics.update(tem)
    ...
    if log_cfg is not None:
        log_metrics_json(log_metrics, log_file)
        end_log(log_file)
    log_config(save_path, {"cfg": cfg, "dataset": dataset})
